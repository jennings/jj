...
fn test_run_restore_descendants_preserves_content() {
    ...
    let output = work_dir
        .run_jj(&[
            ...
            "sh",
            "-c",
            format!("{fake_formatter_path} --tee ran-$JJ_CHANGE_ID.txt").as_str(),
        ])
        .success();
    assert_snapshot!(output.stderr, @r"
    Rewrote 2 commits with sh -c $FAKE_FORMATTER_PATH --tee ran-$JJ_CHANGE_ID.txt
...
#[test]
fn test_run_no_amend_does_not_rewrite() {
    let mut test_env = TestEnvironment::default();
    test_env.run_jj_in(".", ["git", "init", "repo"]).success();
    let fake_formatter = assert_cmd::cargo::cargo_bin("fake-formatter");
    assert!(fake_formatter.is_file());
    ...
    // --no-amend this would amend the commits; with it nothing is rewritten.
    let output = work_dir
        .run_jj(&[
            "run",
            "--no-amend",
        ...
        .success();
    insta::assert_snapshot!(output.stderr, @r"
    Ran command on 1 revisions. No commits were modified (--no-amend).
    Nothing changed.
    [EOF]
    ");
    ...
    let files_a = work_dir
        ...
        .to_string();
    assert!(
        !files_a.contains("touched.txt"),
        "touched.txt must not appear in commit a under --no-amend; got:\n{files_a}",
    );
}

#[test]
fn test_run_no_amend_does_not_leak_between_invocations() {
    // Mirrors test_run_pool_no_file_leak_between_invocations but run 1 uses
    // --no-amend. The pool slot's tree_state is still updated by the snapshot
    ...
    // Run 1 (--no-amend): writes artifact.txt in the pool slot for commit2 but
    ...
    assert!(
        ...
        "artifact.txt must not leak into commit1 from --no-amend slot state; got:\n{files}",
    );
    assert!(
...
#[test]
fn test_run_no_amend_works_on_immutable() {
    let test_env = TestEnvironment::default();
    test_env.run_jj_in(".", ["git", "init", "repo"]).success();
    let work_dir = test_env.work_dir("repo");
    ...
    // Without --no-amend, running on an immutable commit must fail.
    ...
    // With --no-amend the same commit is allowed.
    work_dir
        .run_jj(&["run", "--no-amend", "-r", "main", "--", "true"])
        .success();
}

#[test]
fn test_run_no_amend_conflicts_with_restore_descendants() {
    let test_env = TestEnvironment::default();
    test_env.run_jj_in(".", ["git", "init", "repo"]).success();
    let work_dir = test_env.work_dir("repo");

    let output = work_dir.run_jj(&["run", "--no-amend", "--restore-descendants", "--", "true"]);
    assert!(!output.status.success());
    insta::assert_snapshot!(output.stderr, @r"
    error: the argument '--no-amend' cannot be used with '--restore-descendants'

    Usage: jj run --no-amend <COMMAND> [ARGS]...
